...
pub const DEFAULT_MAX_CELL_INDEX: usize = 99999;
...
fn one_cell_bounds() {
    ...
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), 0);
}

#[test]
fn ptr_increment_bounds() {
    let instrs = parse(">").unwrap();
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), 1);
}

#[test]
fn ptr_increment_sequence_bounds() {
    let instrs = parse(">>.<").unwrap();
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), 2);

    let instrs = parse(">><>>").unwrap();
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), 3);
}
...
fn multiply_move_bounds() {
    ...
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), 4);
}
...
fn unbounded_movement() {
    let instrs = parse("[>]").unwrap();
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), DEFAULT_MAX_CELL_INDEX);
...
fn excessive_bounds_truncated() {
    ...
    let instrs = vec![PointerIncrement {
                          amount: DEFAULT_MAX_CELL_INDEX as isize + 1,
                          position: Some(Position { start: 0, end: 0 }),
                      }];
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), DEFAULT_MAX_CELL_INDEX);
}
...
fn quickcheck_highest_cell_index_in_bounds() {
    fn highest_cell_index_in_bounds(instrs: Vec<AstNode>) -> bool {
        let index = highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX);
        index <= DEFAULT_MAX_CELL_INDEX
    }
...
fn increment_offset_bounds() {
    ...
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), 5);
}
...
fn set_offset_bounds() {
    ...
    assert_eq!(highest_cell_index(&instrs,DEFAULT_MAX_CELL_INDEX), 11);
}
